...
int main() {
	...
	pair<int,int> rect = s->GetResolution();
	//s->SetFullScreen(false);

	float epsilon = 0.1f;
	Window*w = Window::CreateGameWindow("CSC8503 Game technology!", (int) (rect.first + epsilon), (int) (rect.second + epsilon), fullScreen);
	...
	while (w->UpdateWindow() && !Window::GetKeyboard()->KeyDown(KeyboardKeys::ESCAPE) && gsm.GetGameState() != GameStateManager::State::Exit) {
		float dt = w->GetTimer()->GetTimeDeltaSeconds();
		if (dt > 0.1f) {
