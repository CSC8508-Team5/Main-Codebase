...
void GameObject::UpdateBroadphaseAABB() {
	...
	if (boundingVolume->type == VolumeType::AABB) {
		...
		//TODO implent Cap 2 broadphaseAABB
		Matrix3 mat = Matrix3(transform.GetOrientation());
		mat = mat.Absolute();
		Vector3 halfSizes = Vector3(1, 1, 1) * ((CapsuleVolume&)*boundingVolume).GetRadius();
		halfSizes.y = ((CapsuleVolume&)*boundingVolume).GetHalfHeight();
		broadphaseAABB = mat * halfSizes;
	}
}