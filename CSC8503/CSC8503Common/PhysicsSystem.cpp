...
void PhysicsSystem::Update(float dt) {
	if (useBulletPhysics)
	{
		UpdateBullet(dt, numCollisionFrames);
	}
	else
	{
		UpdateRaw(dt);
...
void PhysicsSystem::UpdateRaw(float dt)
{
...
void NCL::CSC8503::PhysicsSystem::IntegrateWorldTransform()
{
	std::vector < GameObject* >::const_iterator first;
	std::vector < GameObject* >::const_iterator last;
	gameWorld.GetObjectIterators(first, last);
	...
	for (auto i = first; i != last; ++i) {
		btCollisionObject* object = (*i)->GetBulletPhysicsObject();
		if (object == nullptr) {
			continue; // No physics object for this GameObject !
		}
...
void NCL::CSC8503::PhysicsSystem::IntegrateBullet()
{
	std::vector < GameObject* >::const_iterator first;
	std::vector < GameObject* >::const_iterator last;
	gameWorld.GetObjectIterators(first, last);
	...
	for (auto i = first; i != last; ++i) {
		...
		Transform& transform = (*i)->GetTransform();
		transform.SetOrientation(Quaternion(bTrans.getRotation()));
		transform.SetPosition(Vector3(bTrans.getOrigin()));
	}
}
